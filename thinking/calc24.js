...
*/
function calc24(ary, resultRecord) {
    var len = ary.length;
    if (len === 1) {
    ...
    // 双重循环，确保任意两个数，可以组合
    for (var i = 0; i < len; i++) { // 第一个数(计算时被两个数结果替换)
        for (var j = i + 1; j < len; j++) { // 第二个数(计算时候被最后一个数替换)
            // 去除第二个数
            var firstNum = ary[i];
            var secondNum = ary[j];
            ary[j] = ary[len - 1];
            // 去除表达式（同上述去除数值逻辑）
            var firstExp = resultRecord[i];
            var secondExp = resultRecord[j];
